...
leading. These results motivate our hypothesis but stop short of testing it
against a generative-augmentation baseline on identical data, which is what we do
here.
...
thickness as a driver of sensitivity. We extend that style of analysis to a
multi-family cladding lattice, where the question is not the thickness of one
layer but which of several hole families controls leakage.
...
model. Establishing that requires testing physics-informed generators, which
would be the natural way to obtain the data volume without the inconsistency.
...
data -- an MSE of 0.121 against 0.314, a 61\% reduction -- while using only the
432 original FV-FEM samples and no synthetic data at all. Under
leave-one-geometry-out validation the advantage holds in both mean and variance,
0.246 $\pm$ 0.226 against 0.825 $\pm$ 0.609.
