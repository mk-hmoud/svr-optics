...
% Bookmark levels: \paragraph and \bmhead sit several levels below \section, so
% hyperref reports "Difference between bookmark levels is greater than one" and
% silently repairs the outline. Limiting the bookmark tree to sections and
% subsections removes the jumps; the PDF outline is more useful for it.
...
\title[Confinement-Loss Prediction in PCF-SPR Sensors]{Confinement-loss prediction in photonic crystal fiber SPR sensors: kernel surrogates versus generative data augmentation}
...
The measurements are plotted in Online Resource 1, Sect.~S8.
...
\textbf{Consent to publish.} Not applicable.
...
\textbf{Author contribution.} M.K.H. implemented the surrogate models, ran the
experiments, prepared the figures and tables, and wrote the machine-learning
sections. C.K. supervised the machine-learning methodology. A.Y. provided the
photonic modelling and FV-FEM dataset, and wrote Sect.~\ref{sec:model}. All
