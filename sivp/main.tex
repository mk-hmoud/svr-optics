...
instability under fabrication error. Timing measurements show the generative
...
Karush-Kuhn-Tucker conditions are given in the Online Supplement, Sect.~S1. The
...
(Online Supplement, Sect.~S3). Augmentation is therefore not introducing noise
...
scatter. Peak jitter grows from 1.03 to 1.34 dataset units between the 1\% and
5\% levels; the complete tables and the per-variable decomposition are given in
the Online Supplement, Sect.~S6.

The pitch $\Lambda$ dominates, accounting for over 90\% of the observed
variance in peak position, while $d_1$ contributes a negligible share. This
establishes a fabrication priority: pitch uniformity along the drawn fiber
governs whether a device matches its calibration, whereas tolerance on the
smallest cladding holes may be relaxed without appreciable spectral cost.

Two limitations qualify these values. First, the observed peak spread nearly
equals the width of the wavelength grid itself, which indicates that the
peak-detection step reaches the grid boundary in a substantial fraction of
trials. The jitter values should therefore be
interpreted as upper bounds and as a ranking rather than as calibrated
magnitudes. Second, the analysis perturbs the surrogate inputs only, and
...
% (1.27 -> 127 nm; see Supplement Table S5).
%
...
analyte index rises from 1.33 to 1.35, in near-uniform steps of 0.279 and 0.273
dataset units, giving a sensitivity of 27.63 per refractive index unit in the
units of the source dataset. The phase-matching condition was never supplied to
...
\textbf{Data availability.} The FV-FEM dataset was generated in the course
of~\cite{zelaci2021} and is available from the authors on reasonable request.
...
manuscript. C.K. supervised the machine-learning methodology. A.Y. provided the
photonic modelling and FV-FEM dataset. All authors revised and approved the
manuscript.
