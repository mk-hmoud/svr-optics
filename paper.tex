...
We performed a 9-fold Leave-One-Group-Out (LOGO) cross-validation, where each fold corresponds to an entire geometric configuration ($N=48$ samples per fold) being withheld from training. This ensures that the model is tested on truly unseen fiber designs. As summarized in Table \ref{tab:cv_results}, the proposed SVR model maintains a low Average MSE of 0.246 across all configurations, significantly outperforming GAN-augmented benchmarks which exhibit a higher error of 0.825 and greater instability (Std Dev = 0.609). The relatively low standard deviation of the SVR model (0.226) confirms its robustness across diverse PCF geometries.
...
\begin{table}[htbp]
\caption{9-Fold Cross-Validation Results (MSE)}
\begin{center}
\begin{tabular}{lcc}
\toprule
\textbf{Fold} & \textbf{Proposed SVR} & \textbf{GAN-Augmented} \\
\midrule
Average & 0.246 & 0.825 \\
Std. Dev. & 0.226 & 0.609 \\
\bottomrule
...
\begin{table}[htbp]
\caption{Benchmarking Proposed Models against Literature (Single Fold)}
\begin{center}
\begin{tabular}{lc}
\toprule
...
ML-Enhanced ANN (Ehyaee et al. \cite{b10}) & 0.140 \\
...
While the proposed Bayesian-optimized SVR model demonstrates high predictive accuracy and physical consistency, this study is subject to certain limitations. Primarily, the results are grounded in numerical investigations using the FV-FEM method. Although the COMSOL simulations utilize high-fidelity boundary conditions and established material data, experimental validation involving the fabrication of the proposed PCF-SPR structure is required to confirm these findings under real-world conditions. Specifically, fabrication-induced imperfections such as surface roughness of the plasmonic Gold layer, material impurities, and slight geometric deviations beyond the modeled manufacturing tolerances may influence the sensor's absolute sensitivity and signal-to-noise ratio. Future work will focus on the experimental implementation of the optimized design to bridge the gap between numerical surrogate modeling and practical sensor deployment.
...
\bibitem{b11} Q. Zhang et al., ``Highly sensitive PCF sensor based on SPR effect for dual-parameter detection of cancer cells and temperature,'' \textit{Physica Scripta}, 2025.
\end{thebibliography}
