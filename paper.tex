...
\author{\IEEEauthorblockN{Mohammad Khaleel Hammoud}
\IEEEauthorblockA{Computer Engineering Department\\
European University of Lefke\\
25140145@eul.edu.tr}
\and
\IEEEauthorblockN{Cem Kalyoncu}
\IEEEauthorblockA{Computer Engineering Department\\
European University of Lefke\\
ckalyoncu@eul.edu.tr}
\and
...
We hypothesize that PCF-SPR confinement loss, with sharp non-linear resonances and small datasets ($N < 500$), is better suited for boundary-focused algorithms like SVR and GPR rather than deep learning. This is supported by Kaium and Mollah \cite{b8} and Opoku et al. \cite{b9}, who validate the efficiency of kernel-based models over ANNs for limited fiber data. We evaluate Bayesian-optimized SVR and GPR against the GAN-ANN baseline, demonstrating that these lightweight models significantly outperform augmented DNN benchmarks. Furthermore, we empirically prove an ``Augmentation Paradox'': microscopic numerical noise in GAN-generated data actively harms predictive performance.
...
The remainder of this paper is organized as follows: Section II describes the numerical modeling. Section III presents the methodology. Section IV discusses performance and robustness. Section V provides engineering validation, and Section VI concludes the paper.
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=.5\linewidth]{Figx}
...
SVR and GPR models achieved superior accuracy without augmentation (Table \ref{tab:benchmark}). To rigorously verify results, 9-fold LOGO cross-validation was performed. As shown in Table \ref{tab:benchmark}, SVR maintains a low Average MSE (0.246), outperforming GAN-augmented benchmarks (0.825) with higher stability (Std Dev = 0.226 vs 0.609).
...
\begin{table}[htbp]
\caption{Model Performance Summary (MSE)}
\begin{center}
\begin{tabular}{lcc}
\toprule
\textbf{Model} & \textbf{Static Split} & \textbf{9-Fold CV (Avg)} \\
\midrule
Baseline DNN \cite{b1} & 0.894 & --- \\
GAN-Augmented DNN \cite{b1} & 0.314 & 0.825 \\
ML-Enhanced ANN \cite{b10} & 0.084 & --- \\
\textbf{Proposed Optimized SVR} & \textbf{0.121} & \textbf{0.246} \\
...
This performance gap highlights a fundamental difference in how these models handle limited training data. While the ANN-enhanced sensor in \cite{b10} achieves a lower MSE of 0.084, it requires a dataset of 1,476 samples. In contrast, our proposed SVR achieves a comparable MSE of 0.121 using only 432 samples---a 70\% reduction in training data requirements.
...
To ensure the proposed model is practically useful for optical optimization, we apply SHAP (SHapley Additive exPlanations) analysis. Building upon contemporary frameworks that utilize SHAP for PCF-SPR design---such as Khatun and Islam's recent optimization of gold thickness \cite{b3}---we extend this interpretability to complex multi-hole geometries. Rather than treating the ML pipeline as a ``black box,'' SHAP deconstructs the regression outputs by isolating the marginal contribution of each physical feature. As illustrated in Fig. \ref{fig:shap}, outer air hole diameters ($d_2, d_3$) are the primary drivers of confinement loss. 
...
\begin{figure*}[t]
    ...
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{feature_importance_bar.png}
        \caption{Global feature importance ranking derived from SHAP values.}
        \label{fig:shap}
    \end{minipage}\hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{spectral_sensitivity_curves.png}
...
We predicted the complete spectral loss response for varying analytes to verify physical consistency. As shown in Fig. \ref{fig:sensitivity}, the model captures resonance peak shifts as $n_a$ increases from 1.33 to 1.36. The average sensitivity is 27.63 nm/RIU, consistent with the base design geometric parameters. This captured redshift confirms the model understands the phase-matching between the fiber mode and SPP, making it a reliable surrogate for rapid optimization.

\subsection{Discussion of Limitations}
This study is grounded in numerical FV-FEM investigations. While high-fidelity, experimental validation is required to confirm findings under real-world conditions. Fabrication-induced imperfections, such as surface roughness of the Gold layer and material impurities, may influence sensitivity and SNR. Future work will focus on experimental implementation to bridge the gap between surrogate modeling and practical deployment.

\section{Conclusion}
In this work, we demonstrate that Bayesian-optimized Support Vector Regression (SVR) and Gaussian Process Regression (GPR) models significantly outperform GAN-augmented neural networks for Photonic Crystal Fiber-Surface Plasmon Resonance (PCF-SPR) sensor modeling. We empirically identify an "Augmentation Paradox," where the non-physical numerical noise inherent to GAN-generated synthetic samples actively harms the predictive performance of kernel-based models, suggesting that small-data regimes in photonics are better suited for boundary-focused algorithms than generative deep learning. 

In terms of computational efficiency, the proposed SVR model offers a significant advantage over previous deep learning approaches. While the GAN-augmented DNN method \cite{b1} requires extensive computational resources for the generative training phase and achieving an MSE of 0.314, our SVR model achieves a superior MSE of 0.121 (a 61\% error reduction) using only the original 432 FV-FEM samples. This eliminates the multi-hour overhead associated with training generative networks and optimizing deep architectures, allowing for near-instantaneous geometric sweeps once the surrogate model is trained.

Furthermore, by integrating SHAP analysis and Monte Carlo simulations, we move beyond "black box" modeling to provide actionable engineering insights. Our robustness analysis identifies the sensor pitch ($\Lambda$) as the primary driver of manufacturing instability, accounting for over 90\% of resonance jitter (1.23 nm at 3\% tolerance). This research underscores that high-accuracy predictive modeling, when paired with explainable AI and robustness analysis, can effectively guide the design of next-generation PCF-SPR sensors while drastically minimizing computational costs and ensuring manufacturing reliability.
...
\bibitem{b8} S. M. A. Kaium and M. A. Mollah, ``Optical properties estimation of photonic crystal fiber using Gaussian process regression,'' \textit{Optics Continuum}, vol. 3, no. 8, pp. 1369--1388, 2024.
\bibitem{b9} G. Opoku et al., ``A comparative analysis of ten machine learning models for performance prediction in a numerically simulated photonic crystal fibre-based surface plasmon resonance sensor,'' \textit{Journal of Electromagnetic Waves and Applications}, 2026.
\bibitem{b10} A. Ehyaee et al., ``Machine learning-enhanced surface plasmon resonance based photonic crystal fiber sensor,'' \textit{Optics Express}, vol. 32, no. 8, pp. 13369--13383, 2024.
\end{thebibliography}
